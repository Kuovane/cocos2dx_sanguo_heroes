...
namespace entityx {
    ...
    static bool ENTITY_USE_ASYNCHRONOUS_DESTROY = true;
...
private:
  ...
  ComponentHandle(EntityManager *manager, Entity::Id id) :
    ...
    {
        //fu.chenhao, cache component data
        cache();
    }
  ...
  Entity::Id id_;
    ...
    //fu.chenhao, cache component data
    Entity::Id m_cachedId;
    C* m_pReferenceData = NULL;
    void cache();
};
...
class EntityManager : entityx::help::NonCopyable {
 public:
  ...
  virtual ~EntityManager();
    ...
    //fu.chenhao, delay destroy entities
    std::vector<Entity> waitToDestroiedEntities;
    void cleanWaitToDestroiedEntities();
...
inline void ComponentHandle<C>::remove() {
  ...
  manager_->remove<C>(id_);
    ...
    //fu.chenhao, cache component data
    template <typename C>
    inline void ComponentHandle<C>::cache() {
        m_cachedId = id_;
        m_pReferenceData = manager_->get_component_ptr<C>(id_);
    }
